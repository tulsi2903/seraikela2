...
// namespace App\Http\Controllers\SchemeReviewDuplicateDataCheckController;
...
use App\Http\Controllers\SchemeReviewDuplicateDataCheckController;
use Auth;
...
class SchemePerformanceController extends Controller
{
            ...
            foreach ($SchemePerformance_details as $key_SchemePerformance => $value_SchemePerformance) {
                ...
                if ($value_SchemePerformance['status'] != "") {
                    ...
                    if ($value_SchemePerformance['status'] == 2) {
                        $to_append_tbody .= '<option value="4"  >Open</option>';
                    ...
                    if ($value_SchemePerformance['status'] == 4) {
                        $to_append_tbody .= '<option value="4" selected >Open</option>';
                        $to_append_tbody .= '<option value="2"  >Sanctioned</option>';
                        $to_append_tbody .= '<option value="0">Inprogress</option>';
                        $to_append_tbody .= '<option value="3">Cancel</option>';
                    }
                } else {
        ...
        */
        ...
        $to_append_row .= '<td>
                           ...
                           <option value="4" selected >Open</option>
                            <option value="2">Sanctioned</option>
                            </select>
                        </td>';
    ...
    public function duplicate_scheme_perfomamce(SchemeReviewDuplicateDataCheckController $duplicate_scheme_perfomamce)
    {
        $uom = 1;
        $distance_to_measure = 10;
        $id = 9;
        return $duplicate_scheme_perfomamce->get_duplicate_scheme_perfomamce($id, $uom, $distance_to_measure);
    }
        ...
        if ($_FILES['excelcsv']['tmp_name']) {
            ...
            if (count($readExcel) != 0) {
                if (count($readExcel) <= 250) {
                    ...
                    {
                        array_pop($excelSheetHeadings);// Removing Last Null INdex
                    }
                            ...
                            if ($row['coordinate'] != "") {
                                ...
                                if ($coordinate_count % 2 == 0) {
                                    ...
                                    foreach ($multiple_array as $key_coordinates => $value_coordinates) {
                                        ...
                                        $coordinate[] = array('latitude' => preg_replace('/\s+/', '', trim(strtolower($value_coordinates[0]))), 'longitude' => preg_replace('/\s+/', '', trim(strtolower($value_coordinates[1]))));
                                    }
                                }
                            }
                            // print_r($coordinate);
                            ...
                            if ($row['sno.'] != null) {
                                ...
                                if ($row['sno.'] != null && $fetch_block_id != null && $fetch_panchayat_id != null && $fetch_year_id != null && $fetch_subdivision_id != null && $scheme_assest_id != null && in_array($panchayat_name, $geo_names_array) && in_array($block_name, $geo_block_names_array)) {
                                    ...
                                    if ($row['sno.'] != null) {
                                        ...
                                        if ($coordinate_error !== 0) {
                                            $ErrorTxt .= " On SNo. " . $row['sno.'] . " Coordinate Upload Unsuccessful \n";
                                        }
                                    }
                                }
                            }
