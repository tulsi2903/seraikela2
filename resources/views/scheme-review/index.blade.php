...
@section('page-style')
    <style>
        ...
        @media print {
            ...
            .card-title{ /* to override layout.blade.php styling */
        ...
        <div class="card"   style="border-top: 3px solid #5167a0;">
            ...
            <div class="card-body">
                <!-- <div class="form-group">    
                    ...
                    <input type="radio" id="review-for-block" name="review_for" value="block" hidden>
                    <input type="radio" id="review-for-panchayat" name="review_for" value="panchayat" hidden checked>
                    <!-- <hr/>
                </div> -->
                <div class="row">
                    <div class="col-6">
                        <div class="form-group">
                            <?php if($review_type=="scheme"){ ?>
                            ...
                            else{ // $review_type==group
                            ?>
                                <label for="group_id">Group<span style="color:red;margin-left:5px;">*</span></label>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control" id="selected_blocks" hidden>
    ...
    <div class="card">
            <div class="card-body"  style="border-top: 3px solid #5167a0; min-height: 500px;">
                ...
                <div id="all-view-details-filter" style="display: none; overflow:hidden; background: #efefef; padding: 10px 10px; border-radius: 5px;">
                    ...
                    <div id="all-view-filter" style="display:inline-block; width: 350px; float:right;">
                        <div class="input-group">
                            <?php if($review_type=="scheme"){ ?>
                                <label style="margin: 8px 10px 0 0;">Scheme Groups</label>
                                <select name="group_id" id="group_id" class="form-control">
                                    <option value="">-All Scheme Groups-</option>
                                    @foreach($scheme_group_datas as $scheme_group_data)
                                        <option value="{{$scheme_group_data->scheme_group_id}}">{{$scheme_group_data->scheme_group_name}}</option>
                                    @endforeach
                                </select>
                            ...
                            ?>
                                <label style="margin: 8px 10px 0 0;">Schemes</label>
                                <select name="scheme_id" id="scheme_id" class="form-control">
                            ...
                            <div class="input-group-append">
                                <button type="button" class="btn btn-primary btn-sm" onclick="search()"><i class="fas fa-search"></i></button>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="tab-content mt-2 mb-3" id="myTabContent">
                    ...
                    <div class="tab-pane fade show active printable-area" id="tabular-view-tab" role="tabpanel">
                        <h4 class="printable-area-title">Tabular View</h4>
                        <div id="tabular-view">
                        </div>
                        <div class="no-data">
                    ...
                    <div class="tab-pane fade printable-area" id="graphical-view-tab" role="tabpanel">
                        <h4 class="printable-area-title">Graphical View</h4>
                    ...
                    <div class="tab-pane fade printable-area" id="map-view-tab" role="tabpanel">
                        <h4 class="printable-area-title">Map View</h4>
                        <div id="map-view">
                            <div class="map-view-form">
                                <div class="row form-group">
                                    ...
                                    <div class="col-3">
                                        <label for="map-view-indicators">Select Indicator<span style="color:red;margin-left:5px;">*</span></label>
                                        <select name="map-view-indicators" id="map-view-indicators" class="form-control">
                                            <option value="">-Select-</option>
                                        </select>
                                        <div class="invalid-feedback">Please select indicator</div>
                                    </div>
                                    <div class="col-2">
...
<script>
    var review_for = 'panchayat'; // block, panchayat
    var review_type = '{{$review_type}}'; // scheme, group
    var selected_panchayat = new Array;
    var panchayat_form_data_received = false;

    $(document).ready(function(){
        review_for_toggle(); //initializing panchayat
...
<script>
    ...
    // }).mouseover();
    $("g").click(function(){
        var block_id_tmp= $(this).children('path').data('geo-id').toString();
        if(selected_blocks.includes(block_id_tmp)){
            $(this).children('path').removeClass("active");
            selected_blocks.splice(selected_blocks.indexOf(block_id_tmp), 1);
            $("#selected_blocks").val(selected_blocks);
        }
        else{
            $(this).children('path').addClass("active");
            selected_blocks.push(block_id_tmp);
            $("#selected_blocks").val(selected_blocks);
        }
    ...
    $(document).ready(function(){
        ...
        $("#selected_blocks").change(function(){
            if($("#selected_blocks").val()){
                $("#selected_blocks").removeClass('is-invalid');
            }
        });
    });
    ...
    function search(){
        var scheme_group_id_error = true; // depend on review_type (scheme/group)
        var year_id_error = true;
        var selected_blocks_error = true;
        ...
        if(review_type=="scheme")
        {
            if($("#scheme_id").val()==""){
                ...
                scheme_group_id_error=true;
            }
            else{
                $("#scheme_id").removeClass('is-invalid');
                scheme_group_id_error=false;
            }
        }
        else{ // group
            if($("#group_id").val()==""){
        ...
        // block selected
        if($("#selected_blocks").val()==""){
            $("#selected_blocks").addClass('is-invalid');
            selected_blocks_error=true;
        }
        else{
            $("#selected_blocks").removeClass('is-invalid');
            selected_blocks_error=false;
        }
        ...
        if(!scheme_group_id_error&&!year_id_error&&!selected_blocks_error&&!panchayat_id_error){
    ...
    function  getDatas(){
        ...
        group_id_tmp = $("#group_id").val(); //$("#group_id").val();

        year_id_tmp = $("#year_id").val();
        selected_blocks_tmp = $("#selected_blocks").val(); // string, have convert to array in controller// block ids

        panchayat_id_tmp = "";// string, panchayat data if review_for==panchayat
        ...
        $.ajax({
            url: "{{url('scheme-review/get-datas')}}",
            data: {'review_for': review_for, 'scheme_id': scheme_id_tmp, 'group_id': group_id_tmp, 'selected_blocks': selected_blocks_tmp, 'panchayat_id': panchayat_id_tmp, 'year_id': year_id_tmp},
            method: "GET",
            ...
            success: function (data){
                ...
                if(data.response=="no_data"){ // no data found
                    ...
                    if(review_type=="scheme"){
                        ...
                        $("#all-view-details").html("<b>Group: </b>"+$("#group_id option:selected").text());
                    }
    ...
    function getPanchayatDatas(){
        selected_blocks_tmp = $("#selected_blocks").val(); // string, have convert to array in controller // blocks id's
        $.ajaxSetup({
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            }
        ...
        $.ajax({
            url: "{{url('scheme-review/get-panchayat-data')}}",
            data: {'selected_blocks': selected_blocks_tmp},
            method: "GET",
            contentType: 'application/json',
            dataType: "json",
            beforeSend: function(data){
...
<script>
    function initializeTabularView(data){
        ...
        var toShowTabularForm = "<table class='table table-bordered table-datatable table-sm'>";
        var block_name_tmp = "NA";

        for(var i=0; i<data.length; i++){
            if(i==0){ // for first row
                toShowTabularForm = toShowTabularForm + "<tr style='background: #d6dcff; color: #000;'>";
            }
            else{ // for others
            ...
            for(var j=0;j<data[0].length; j++){
                if(i==0){  // for first row i.e th
                    ...
                    if(block_name_tmp!=data[i][1]){ // for block name (entire row)
                        block_name_tmp = data[i][1];
                        toShowTabularForm = toShowTabularForm + "<td colspan='"+data[0].length+"' class='table-secondary'><center><b>Block:</b> "+data[i][1]+"</center></td>";
                        i--;
                        break;
                    }
                    else{
...
<script>
    function initializeMapView(map_geo_related, map_view_blocks, map_view_indicators){
        if(map_geo_related==0) {
            $("#map-tab").hide();
        }
        else{
            $("#map-tab").show();
            ...
            $("#map-view-indicators").html('<option value="">-Select-</option>');
            for(var i=0; i<map_view_indicators.length; i++){
                $("#map-view-indicators").append('<option value="'+map_view_indicators[i].id+'">'+map_view_indicators[i].name+'</option>');
            }
    ...
    $(document).ready(function(){
        ...
        $("#map-view-indicators").change(function(){
            $(this).removeClass("is-invalid");
        });
    });
    function mapSearch(){
        ...
        var mapViewScheme = $("#scheme_id").val();
        var mapViewPanchayat = $("#map-view-block").val();
        var mapViewIndicators = $("#map-view-indicators").val();
        ...
        if(mapViewIndicators == ""){
            $("#map-view-indicators").addClass("is-invalid");
            errorMapSearch = true;
        }
        else{
            $("#map-view-indicators").removeClass("is-invalid");
        }
        ...
        if(errorMapSearch==false){
            ...
            $.ajax({
                url: "{{url('scheme-review/get-map-data')}}",
                data: {'review_for': review_for, 'geo_id': mapViewPanchayat, 'scheme_id': mapViewScheme, 'indicator_id': mapViewIndicators, 'year_id': mapViewYear},
                method: "GET",
                contentType: 'application/json',
                dataType: "json",
                beforeSend: function(data){
    ...
    function showMap(data) {
        var icon = {
            url: '{{url("public/images/home.png")}}', // url
        ...
        $.each(data, function(){
            ...
            '<div id="siteNotice">'+
            '</div>'+
            '<h4 id="firstHeading" class="firstHeading">'+this.location_address+'</h4>'+
            // '<div id="bodyContent">'+
