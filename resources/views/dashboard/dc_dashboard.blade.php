...
<style>
    ...
    #scheme-performane-table th,
    #scheme-performane-table td {
        border: 1px solid black;
        padding: 5px !important;
    }

    #scheme-performane-table tr td:first-child,
...
<div class="row">
    <div class="col-md-12" style="margin-top:1em;">
        <div class="card">
            <div class="card-head-row card-tools-still-right" style="background:#fff;">
                <h4 style="color: #000;font-size: 1.2em;padding: 1em; overflow: hidden;">
                    <div style="display: inline-block; float: left; width:50%;font-family: 'Bree Serif', serif;">{{$phrase->scheme_Performance}}</div>
                    <!-- (monthly) -->
        ...
        <div class="card" style="min-height: 600px;">
            ...
            <div class="card-body">
                ...
                <div style="padding: 20px;text-align:center; max-width:550px;margin:auto;margin-top: -47px;">
                    <svg id="block-performance-svg-map" viewBox="0 0 739 752" stroke-linecap="round" stroke-linejoin="round">
                        ...
                        <g data-info="Kharsawan" data-geo-id="5" data-panchayat-target="kharsawan">
                            ...
                            <text x="300" y="420">{{$phrase->kharsawan}}</text>
                        </g>
...
<script>
    // for map contents (percentage wise colour)
    $(document).ready(function(){
        $.ajax({
            url: "{{url('dashboard/get-block-performance-percentage-data')}}",
            data: { 'ok': "ok" },
            method: "GET",
            contentType: 'application/json',
            dataType: "json",
            ...
            success: function (data){
                ...
                data.forEach(function(element){
                    ...
                    var percentage = Number(element.percentage);
                    if(percentage==0){ fill_color="#ffdad3"; }
                    else if(percentage<30){ fill_color="#ef8122"; }
                    else if(percentage<70){ fill_color="#f6ce1c"; }
                    else if(percentage==100){ fill_color="#8cc441"; }
                    else{ fill_color="#d5df28"; }

                    // stroke_color changing
                    $("#block-performance-svg-map g[data-geo-id='"+element.geo_id+"'] path").css('fill',fill_color);
                });
            }
        });
    })
    ...
    <script>
        function get_year(e) {
            ...
            // alert(year_value);
            $("#Scheme_Performance").html("");
            $.ajax({
                url: "{{url('dashborad/schemeperformance/')}}" + "/" + year_value,
                method: "GET",
                contentType: 'application/json',
                dataType: "json",
                beforeSend: function (data) {
                ...
                success: function (data) {
                    ...
                    var toAppend = "";
                    if (data != "") {
                        if (data.dashboard_scheme_performance_has_datas == "success") {
                            toAppend = `<table class="table" id="scheme-performane-table">
                            <thead style="background: #d6dcff;color: #000;">
                                <tr>`;
                            var value = new Array();
                            for (i = 0; i < data.performance_table_heading_1.length; i++) {
                                if (i != 0) {
                                    toAppend += `<th colspan='3' style='text-align: center'>`;
                                    value = data.performance_table_heading_1[i].split('::');
                            ...
                            for (i = 0; i < data.performance_table_heading_2.length; i++) {
                                toAppend += `<th>` + (data.performance_table_heading_2[i]) + `</th>`;
                            }

                            toAppend += `</tr>
                            </thead>
                            <tbody>`;
                            for (i = 0; i < data.performance_table_datas.length; i++) {
                                toAppend += `<tr>`;
                                for (j = 0; j < data.performance_table_datas[i].length; j++) {
                                    value = data.performance_table_datas[i][j].split(":");
                                    if (value.length == 2) {
                                        value[1] = Number(value[1]);
                                        if (value[1] == 0) {
                                            toAppend += `<td style='background: #ffcfcf;'>`;
                                        }
                                        else if (value[1] < 30) {
                                            toAppend += `<td style='background: #ff9999;'>`;
                                        }
                                        else if (value[1] < 70) {
                                            toAppend += `<td style='background: #ffcfcf;'>`;
                                        }
                                        else if (value[1] == 100) {
                                            toAppend += `<td style='background: #2cbd36;'>`;
                                        }
                                        else {
                                            toAppend += `<td style='background: #87f387;'>`;
                                        }
                                    }
                                    else {
                                        toAppend += `<td>`;
                            ...
                            toAppend += `  <h4><span style="color: #09c521;"><i class="fas fa-info"></i></span>&nbsp;&nbsp;` + data.dashboard_scheme_performance_has_datas + `"</h4>"`;
                        }
                        $(".custom-loader").fadeOut(300);
                        $("#Scheme_Performance").html(toAppend);
